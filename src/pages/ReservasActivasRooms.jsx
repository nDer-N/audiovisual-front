...
export default function ReservasActivasRooms() {
    ...
    async function getActive() {
        const res = await fetch("https://equipo1.ralejandro.com/api/reservas/salones/active");
        ...
        async function loadNames() {
            ...
            for (const item of elements) {
                if (!roomNames[item.roomId]) {
                    map[item.roomId] = await getName(item.roomId);
                }
            }
            if (Object.keys(map).length > 0)
                setRoomNames(prev => ({ ...prev, ...map }));
        }
